...
const AdminDash = () => {
    ...
    const fetchUsers = async () => { 
        fetch('/api/admin/mark/employees')
    ...
    const fetchDates = async (userId) => {
        const res = await axios.get('/api/admin/mark/dates?user=' + userId)
    ...
    useEffect(() => {
        ...
        setDisabled(false);
        setActivity(groupedData[selectedDate]);
    }, [selectedDate])
    ...
    const onSubmit: SubmitHandler<FieldValues> = async (data) => {
	    ...
	    if(isNaN(postData.score)){
		return toast.error("Please mention a score!")
	    }

            if(postData.score > 25){
                return toast.error("The score cannot be higher than 25!")
            }

            if(postData.score < 0){
                return toast.error("The score cannot be negative!")
            }

            const res = await axios.post('/api/admin/mark/score', postData);
