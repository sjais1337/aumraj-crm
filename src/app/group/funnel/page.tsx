...
const dataSource: IDatasource = {
  getRows: async (params) => {
    ...
    const res = await axios.post('/api/group/funnel/fetch', postData);
    if(res.status == 401){
...
export default function AdminFunnelReport(){
  ...
  const updateFunnel = async (params: any) => {
    ...
    const response = await axios.post('/api/group/funnel/update', {
      funnelId: funnelId,
      field: field,
      value: value
    })
  ...
  async function fetchGraph(filters){
    const res = await axios.post('/api/group/funnel/charts', { 
      filters: filters
    });
  ...
  useEffect(() => {
    const func = async () => {
      const res = await axios.get('/api/group/funnel/fetch/summary');
  ...
  return (
    <>
      ...
      <div className="col-span-1 mb-8 md:col-span-8 border border-stroke mt-8 bg-white px-4 py-4 md:px-4 md:py-4 shadow-default dark:border-strokedark dark:bg-boxdark overflow-x-auto">
        <table className="table-auto w-full text-black dark:text-white">
          <tbody className="">
            <tr >
